...
// const port = 7000; //server runs on port 8000
...
app.use(express.urlencoded({ extended: true })); //middleware that parses the incoming data
...
app.use(
  session({
    name: "fakebook", //cookie name
    secret: "OQ7HPmU769H7gknLjIqMxKLeVSWGfylt", //encryption key
    saveUninitialized: true, //when user is not logged in no data is saved in cookie
    resave: true, //we do not want to rewrite session cookie if it is not changed
    cookie: {
      maxAge: 1000 * 60 * 60 * 5, //age of the cookie in miliseconds --> 3 hours
    },
    store: MongoStore.create({
      mongoUrl: "mongodb://localhost/fakebook",
    }),
  })
);
